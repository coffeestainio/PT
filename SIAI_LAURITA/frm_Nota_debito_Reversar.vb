...
Public Class frm_Nota_debito_Reversar
    ...
    Public Sub Identifica_Movimiento()
        ...
        If NC.Rows.Count = 0 Then
            MessageBox.Show("Nota de Débito No Existe", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
        ...
        With NC.Rows(0)
            If .Item("anulado") Then
                MessageBox.Show("Nota de Débito Ya Fue Reversada", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtid_movimiento.Focus()
                SendKeys.Send("{home}+{end}")
                Exit Sub
            End If
            ...
            '    MessageBox.Show("Nota de Débito Ya Fue Aplicada", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
            '    txtid_movimiento.Focus()
            '    SendKeys.Send("{home}+{end}")
            '    Exit Sub
            'End If
    ...
    Private Sub btnaceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnaceptar.Click
        Try
            If Val(txtid_movimiento.Text) = 0 Then
                MessageBox.Show("Debe Escribir un Número de Nota de Débito", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtid_movimiento.Focus()
            ...
            If movimiento_ID <> txtid_movimiento.Text Then
                MessageBox.Show("Escriba de Nuevo el Número de Nota de Débito y Presione Enter", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtid_movimiento.Focus()
                SendKeys.Send("{home}+{end}")
                Exit Sub
            End If
